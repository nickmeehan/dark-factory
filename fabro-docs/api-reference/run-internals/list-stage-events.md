...
````yaml /api-reference/fabro-api.yaml get /api/v1/runs/{id}/stages/{stageId}/events
openapi: 3.1.0
info:
  title: Fabro Run API
  version: 0.2.0
...
paths:
  /api/v1/runs/{id}/stages/{stageId}/events:
    get:
      ...
      description: >-
        Returns a paginated JSON list of stored run events scoped to a single
        stage visit.
...
components:
  parameters:
    ...
    StageId:
      ...
      required: true
      description: >-
        Identifier of a stage within a run's workflow graph, serialized as
        `node_id@visit`.
      schema:
        type: string
      example: code@2
