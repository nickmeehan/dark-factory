...
````yaml /api-reference/fabro-api.yaml post /api/v1/runs/{id}/deny
openapi: 3.1.0
info:
  title: Fabro Run API
  version: 0.2.0
...
paths:
  /api/v1/runs/{id}/deny:
    post:
      ...
      summary: Deny Run
      description: >-
        Denies a pending run that requires pre-execution approval and fails it
        with `approval_denied`.
      operationId: denyRun
      parameters:
    ...
    DenyRunRequest:
      ...
      properties:
        reason:
          type: string
          description: >-
            Optional human-readable reason for denying execution. Empty or
            whitespace-only values are stored as absent.
